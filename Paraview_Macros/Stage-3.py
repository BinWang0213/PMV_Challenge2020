...
Title.Text = 'Digital rock visulization: Grain'
...
for i in range(nb_frames):
   ...
   slice_I=interpPlaneLoc(i,nb_frames,bbox,plane=SlicePlane,inverse=False)
   #print(i,SlicePlane,slice_I)
   setCliperRange(slicer,bbox,plane=SlicePlane,loc_range=[slice_I,bbox[1]])
...
print('Stage 3.1 End @',renderView.CameraPosition,'FrameID=',frameID)
...
#Fade-in effect for Pore Space
frameID=FadeEffect(animationScene, 
           input_objs_display=[poreDisplay], 
           start_opacitys=[poreDisplay.Opacity],
           end_opacitys=[1.0], 
           nframes=nb_frames,frameID=frameID,
           saveAnimation=True,output_dir=out_dir)
...
#Reset view
Show(image_binary)
#image_binary_Display.SetRepresentationType('Surface')
